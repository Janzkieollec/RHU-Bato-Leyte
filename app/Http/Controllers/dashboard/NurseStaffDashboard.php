...
class NurseStaffDashboard extends Controller
{
    public function nurse()
    {
        return view('content.dashboard.nurse-dashboard');
    }

    public function staff()
    {
        return view('content.dashboard.staff-dashboard');
    }
    ...
    {
        // Count the total number of patients for consultation purposes
        $totalPatients = Patient::count();
    
        // Count all entries in ConsultationAnalytics
        $totalConsultation = ConsultationAnalytics::count(); // Total consultations
        $totalDental = Dental::count();
        ...
        return response()->json([
            ...
            'totalDental' => $totalDental,
        ]);
    }
    
     
    public function getDentalCount()
    {
        $totalPatients = Patient::count();
    ...
    public function getDiagnosisData(Request $request)
    {
        ...
        if ($user->role === 'Nurse' || $user->role === 'Midwife') {
            $diagnosisType = 1; // Consultation
        } elseif ($user->role === 'Staff') {
            $diagnosisType = 2; // Dental
        }
        ...
        $dataQuery = DiagnosisAnalytics::with('diagnosis')
            ...
            ->where('diagnosis.diagnosis_type', $diagnosisType) // Filter for consultation or dental
    ...
    public function getHighestCases(Request $request)
    {
        ...
        $query = DiagnosisAnalytics::with('diagnosis')
            ...
            ->where('diagnosis.diagnosis_type', $diagnosisType) // Filter for diagnosis_type 1
    ...
    public function getAgeDistributionData(Request $request)
    {
        ...
        $query->where('diagnosis.diagnosis_type', $diagnosisType);
    ...
    public function getYearlyConsultationData()
    {
        // Query the consultations and group by year
        $consultationData = Consultation::selectRaw('YEAR(created_at) as year, COUNT(*) as total')
                                        ->groupBy('year')
                                        ->orderBy('year', 'asc')
                                        ->get();

        // Format the data to return
        $years = $consultationData->pluck('year')->toArray();
        $consultationCounts = $consultationData->pluck('total')->toArray();

        // Return the data to be used in the view
        return response()->json([
            'labels' => $years,       // Years for x-axis
            'data' => $consultationCounts  // Total consultations per year
        ]);
    }
    ...
    public function getYearlyDentalData()
    {
        // Query the consultations and group by year
        $dentalData = Dental::selectRaw('YEAR(created_at) as year, COUNT(*) as total')
                                        ->groupBy('year')
                                        ->orderBy('year', 'asc')
                                        ->get();

        // Format the data to return
        $years = $dentalData->pluck('year')->toArray();
        $dentalCounts = $dentalData->pluck('total')->toArray();

        // Return the data to be used in the view
        return response()->json([
            'labels' => $years,       // Years for x-axis
            'data' => $dentalCounts  // Total consultations per year
        ]);
    }
    ...
    public function getDiagnosisAnalytics(Request $request)
    {
        ...
        $dataQuery = DiagnosisAnalytics::with('diagnosis')
            ...
            ->where('diagnosis.diagnosis_type', $diagnosisType) 
